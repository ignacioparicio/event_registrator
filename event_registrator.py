...
def send_alert():
    ...
    email_msg = config["email"]["body_register_intro"] + f"\n\nNew class: {OPT_TEXT}\n\n"\
                + \
                config["email"]["body_register_how_to"] + config["email"][
                    "auto_msg_flag"]
...
def register():
    ...
    try:
        ...
        send_email(
                config["registree"]["email_address"] +
            config["email"]["body_error"])
